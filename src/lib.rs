...
fn mut_option_shards_to_mut_slices<'a>(shards: &'a mut [Option<Shard>]) -> Vec<&'a mut [u8]> {
    let mut result: Vec<&mut [u8]> = Vec::with_capacity(shards.len());
...
impl ParallelParam {
    pub fn new(bytes_per_encode: usize, shards_per_encode: usize) -> ParallelParam {
        ParallelParam {
            bytes_per_encode,
            shards_per_encode,
        }
...
impl<'a> ShardByShard<'a> {
    ...
    fn sbs_encode_shard_checks(&mut self, shards: &mut [Shard]) -> Result<(), SBSError> {
        fn internal_checks(codec: &ReedSolomon, shards: &mut [Shard]) -> Result<(), Error> {
    ...
    fn sbs_encode_checks(&mut self, slices: &mut [&mut [u8]]) -> Result<(), SBSError> {
        fn internal_checks(codec: &ReedSolomon, slices: &mut [&mut [u8]]) -> Result<(), Error> {
    ...
    fn sbs_encode_shard_sep_checks(
        &mut self,
        data: &[Shard],
        parity: &mut [Shard],
    ) -> Result<(), SBSError> {
    ...
    pub fn encode(&mut self, slices: &mut [&mut [u8]]) -> Result<(), SBSError> {
        ...
        self.codec.encode_single(self.cur_input, slices).unwrap();
    ...
    /// Returns `SBSError::TooManyCalls` when all input data shards
    /// have already been filled in via `encode` or `encode_shard`.
    pub fn encode_sep(&mut self, data: &[&[u8]], parity: &mut [&mut [u8]]) -> Result<(), SBSError> {
    ...
    pub fn encode_shard(&mut self, shards: &mut [Shard]) -> Result<(), SBSError> {
        ...
        self.codec
            .encode_single_shard(self.cur_input, shards)
            .unwrap();
    ...
    pub fn encode_shard_sep(
        ...
        self.codec
            .encode_single_shard_sep(self.cur_input, &data[self.cur_input], parity)
            .unwrap();
...
impl ReedSolomon {
    ...
    pub fn new(data_shards: usize, parity_shards: usize) -> Result<ReedSolomon, Error> {
        Self::with_pparam(data_shards, parity_shards, ParallelParam::with_default())
    }
    ...
    fn code_some_slices(&self, matrix_rows: &[&[u8]], inputs: &[&[u8]], outputs: &mut [&mut [u8]]) {
        for i_input in 0..self.data_shard_count {
            self.code_single_slice(matrix_rows, i_input, inputs[i_input], outputs);
        }
    }

    fn code_single_row(
        &self,
        matrix_row_to_use: u8,
        i_input: usize,
        input: &[u8],
        output: &mut [u8],
    ) {
    ...
    fn code_single_slice(
        ...
        if outputs.len() >= self.pparam.shards_per_encode {
            outputs
                .into_par_iter()
                .enumerate()
                .for_each(|(i_row, output)| {
                    self.code_single_row(matrix_rows[i_row][i_input], i_input, input, output);
                })
        } else {
    ...
    fn check_some_slices_with_buffer(
        &self,
        matrix_rows: &[&[u8]],
        inputs: &[&[u8]],
        to_check: &[&[u8]],
    ...
    pub fn encode_single_shard(&self, i_data: usize, shards: &mut [Shard]) -> Result<(), Error> {
        let mut slices: SmallVec<[&mut [u8]; 32]> = convert_2D_slices!(shards =>into SmallVec<[&mut [u8]; 32]>,
    ...
    pub fn encode_single_shard_sep(
        ...
        let mut parity: SmallVec<[&mut [u8]; 32]> = convert_2D_slices!(parity =>into SmallVec<[&mut [u8]; 32]>,
                               SmallVec::with_capacity);
    ...
    pub fn encode_shards(&self, shards: &mut [Shard]) -> Result<(), Error> {
        let mut slices: SmallVec<[&mut [u8]; 32]> = convert_2D_slices!(shards =>into SmallVec<[&mut [u8]; 32]>,
                               SmallVec::with_capacity);
    ...
    pub fn encode_shards_sep(&self, data: &[Shard], parity: &mut [Shard]) -> Result<(), Error> {
        let data: SmallVec<[&[u8]; 32]> = convert_2D_slices!(data   =>into SmallVec<[&[u8]; 32]>,
                               SmallVec::with_capacity);
    ...
    ///
    /// It is recommended to use the `ShardByShard` bookkeeping struct instead of this method directly.
    pub fn encode_single(&self, i_data: usize, slices: &mut [&mut [u8]]) -> Result<(), Error> {
    ...
    /// The slots where the parity shards sit at will be overwritten.
    ///
    pub fn encode(&self, slices: &mut [&mut [u8]]) -> Result<(), Error> {
        check_piece_count!(all => self, slices);
        check_slices!(multi => slices);
        ...
        let input = convert_2D_slices!(mut_input =>into SmallVec<[&[u8]; 32]>,
                               SmallVec::with_capacity);
    ...
    /// The slots where the parity shards sit at will be overwritten.
    ///
    pub fn encode_sep(&self, data: &[&[u8]], parity: &mut [&mut [u8]]) -> Result<(), Error> {
    ...
    pub fn verify_shards(&self, shards: &[Shard]) -> Result<bool, Error> {
    ...
    pub fn verify_shards_with_buffer(
        ...
        let slices = convert_2D_slices!(shards =>to SmallVec<[&[u8]; 32]>,
                               SmallVec::with_capacity);

        let mut buffer = convert_2D_slices!(buffer =>to_mut SmallVec<[&mut [u8]; 32]>,
                               SmallVec::with_capacity);
    ...
    ///
    /// This is a wrapper of `verify_with_buffer`.
    pub fn verify(&self, slices: &[&[u8]]) -> Result<bool, Error> {
        check_piece_count!(all => self, slices);
        check_slices!(multi => slices);
    ...
    pub fn reconstruct(
        ...
        self.reconstruct_internal(slices, slice_present, false)
    }
    ...
    pub fn reconstruct_data(
        ...
        self.reconstruct_internal(slices, slice_present, true)
    }
    ...
    /// `reconstruct`, `reconstruct_data`, `reconstruct_shards`,
    /// `reconstruct_data_shards` share the same core code base.
    pub fn reconstruct_shards(&self, shards: &mut [Option<Shard>]) -> Result<(), Error> {
    ...
    /// `reconstruct`, `reconstruct_data`, `reconstruct_shards`,
    /// `reconstruct_data_shards` share the same core code base.
    pub fn reconstruct_data_shards(&self, shards: &mut [Option<Shard>]) -> Result<(), Error> {
        self.reconstruct_shards_internal(shards, true)
    }

    fn reconstruct_shards_internal(
        &self,
        ...
        let mut shard_present: SmallVec<[bool; 32]> = SmallVec::with_capacity(shards.len());
        ...
        {
            let mut slices = mut_option_shards_to_mut_slices(shards);
            ...
            self.reconstruct_internal(&mut slices, &shard_present, data_only)
                .unwrap();
        }
    ...
    fn reconstruct_internal(
        ...
        // `self.matrix`.
        let mut sub_shards: SmallVec<[&[u8]; 32]> = SmallVec::with_capacity(self.data_shard_count);
        ...
        if data_only {
            ...
            for i_slice in self.data_shard_count..self.total_shard_count {
                if !slice_present[i_slice] {
                    matrix_rows.push(parity_rows[i_slice - self.data_shard_count]);
                }
            }
            {
                ...
                let mut all_data_slices: SmallVec<[&[u8]; 32]> =
                    SmallVec::with_capacity(self.data_shard_count);
                for i_slice in 0..self.data_shard_count {
                    let slice = if slice_present[i_slice] {
                        let result: &[u8] = sub_shards[i_old_data_slice];
                        i_old_data_slice += 1;
                        result
                    } else {
                        let result: &[u8] = missing_data_slices[i_new_data_slice];
                        i_new_data_slice += 1;
                        result
                    };
